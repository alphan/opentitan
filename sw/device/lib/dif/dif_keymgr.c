...
static bool is_idle(const dif_keymgr_t *keymgr) {
  ...
  if (!bitfield_bit32_read(reg_cfg_regwen, KEYMGR_CFG_REGWEN_EN_BIT)) {
    return false;
  }
...
/**
 * Parameters for starting an operation.
 *
...
dif_keymgr_result_t dif_keymgr_configure(const dif_keymgr_t *keymgr,
                                         const dif_keymgr_config_t *config) {
  if (keymgr == NULL || config == NULL) {
    return kDifKeymgrBadArg;
  }
...
dif_keymgr_lockable_result_t dif_keymgr_generate_versioned_key(
  ...
  switch (params.dest) {
    case kDifKeymgrVersionedKeyDestSw:
      hw_op_params.dest = KEYMGR_CONTROL_DEST_SEL_VALUE_NONE;
      hw_op_params.op = KEYMGR_CONTROL_OPERATION_VALUE_GENERATE_SW_OUTPUT;
      break;
    case kDifKeymgrVersionedKeyDestAes:
      hw_op_params.dest = KEYMGR_CONTROL_DEST_SEL_VALUE_AES;
      hw_op_params.op = KEYMGR_CONTROL_OPERATION_VALUE_GENERATE_HW_OUTPUT;
      break;
    case kDifKeymgrVersionedKeyDestHmac:
      hw_op_params.dest = KEYMGR_CONTROL_DEST_SEL_VALUE_HMAC;
      hw_op_params.op = KEYMGR_CONTROL_OPERATION_VALUE_GENERATE_HW_OUTPUT;
      break;
    case kDifKeymgrVersionedKeyDestKmac:
      hw_op_params.dest = KEYMGR_CONTROL_DEST_SEL_VALUE_KMAC;
      hw_op_params.op = KEYMGR_CONTROL_OPERATION_VALUE_GENERATE_HW_OUTPUT;
      break;
    default:
      return kDifKeymgrLockableBadArg;
...
dif_keymgr_result_t dif_keymgr_clear_sideload_keys(const dif_keymgr_t *keymgr) {
  ...
  mmio_region_write32(
      ...
      bitfield_bit32_write(0, KEYMGR_SIDELOAD_CLEAR_VAL_BIT, true));
  mmio_region_write32(
      keymgr->params.base_addr, KEYMGR_SIDELOAD_CLEAR_REG_OFFSET,
      bitfield_bit32_write(0, KEYMGR_SIDELOAD_CLEAR_VAL_BIT, false));
