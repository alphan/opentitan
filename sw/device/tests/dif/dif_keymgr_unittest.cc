...
TEST_P(BadArgsTwo, Configure) {
  auto keymgr = GetGoodBadPtrArg<dif_keymgr_t>(std::get<0>(GetParam()));
  auto config = GetGoodBadPtrArg<dif_keymgr_config_t>(std::get<1>(GetParam()));

  EXPECT_EQ(dif_keymgr_configure(keymgr, config), kDifKeymgrBadArg);
}
...
struct GetStatusCodesTestCase {
   ...
   */
  std::initializer_list<mock_mmio::BitField> reg_val;
  /**
...
TEST_F(SideloadClearTest, BadArg) {
  EXPECT_EQ(dif_keymgr_clear_sideload_keys(nullptr), kDifKeymgrBadArg);
}

TEST_F(SideloadClearTest, Clear) {
  ...
  EXPECT_EQ(dif_keymgr_clear_sideload_keys(&keymgr_), kDifKeymgrOk);
}
